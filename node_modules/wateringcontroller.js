...
function WateringController(config){
	...
	this.openVelve = function(){
		...
		setTimeout(function(){self.emit(self.name,'refresh')},500);
	...
	this.sensorEventHandler = function(data){
		...
		if(data<this.borderValue){
			...
			this.watering(now);
				...
				console.log("border value "+this.borderValue)
	...
	this.getFromConfigDb({column:'dryness',adjust:0,res:null});
	this.getFromConfigDb({column:'duration',adjust:0,res:null});
}
