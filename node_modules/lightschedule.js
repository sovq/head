...
function getSunDataFromDB(isoDateTime,callback){
	var lightingList = [];
	

	sun.find({date: isoDateTime}, function (err, docs) {
...
function scheduler(lightSwitch,EventEmitter) {
	...
	this.sockets = [];
	var db = require('nedb');
	this.config = new db({ filename: '/home/pi/greenhouse/config.db', autoload: true });
	this.status=null
	...
	this.getSundata=function(){
		...
		var self = this;
		getSunDataFromDB(todayDB,function(data){self.getSundataCallBack(self,data)})
	}
