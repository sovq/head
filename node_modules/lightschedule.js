...
function Scheduler(ssrSwitch, configdb, sunsetdb) {
	this.name = "lightScheduler"
	this.switchOffTime = '21:00';
	this.lightSwitch = ssrSwitch;
	this.interval = null;
	...
	this.switchEventHandler=function(data){
		var toggle = data.schedulerState;
		var db = this.config;
		if(toggle == 'on'){
			this.enable();
			this.saveConfigDB(db,'on');
			this.emit(this.name,{'schedulerState':toggle})			
		}else if (toggle == 'off'){
			this.disable();
			this.saveConfigDB(db,'off');
			this.emit(this.name,{'schedulerState':toggle})
		}else if(toggle == 'status'){
			this.checkStatus();
	...
	this.enableCallBack=function(scheduler,data){
		...
		scheduler.emit(scheduler.name,{'schedulerState':toggle});
	}
	...
	this.on('sundatareceived',function(data){
		var time = data.time;
	...
	this.getSundataCallBack=function(scheduler,data){
		...
		scheduler.emit('sundatareceived',{'time':time,'scheduler':scheduler})
	}
